...
const app = new Hono<AppEnv>().use('*', async (c, next) => { c.set('agentId', 'owner'); await next(); }).route('/notifications', notifications);
...
describe('notifications API', () => {
  it('lists using the authenticated owner', async () => {
    ...
    expect(mocks.listNotifications).toHaveBeenCalledWith('owner');
  });
  it('marks only the requested versions using the authenticated owner', async () => {
    expect((await read({ items: [item] })).status).toBe(200);
    expect(mocks.markNotificationsRead).toHaveBeenCalledWith('owner', [item]);
  });
