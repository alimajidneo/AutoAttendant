...
export const microsoftOAuthCallback = new Hono().get("/callback", async c => {
  ...
  if (!state || !code || c.req.query("error")) {
    return c.redirect(settingsUrl("error", "Microsoft authorization was not completed."));
  }
  try {
    ...
    await connectMicrosoftCalendarAccount(state.agentId, refreshToken, account);
    return c.redirect(settingsUrl("connected", undefined, state.agentId));
  } catch {
    console.error("[microsoft-calendar] OAuth callback failed");
    return c.redirect(settingsUrl("error", "Microsoft Calendar connection failed. Try again and approve Calendar access.", state.agentId));
  }
});
