...
export const calendarOAuthCallback = new Hono().get("/callback", async c => {
  ...
  if (!state || !code || c.req.query("error")) {
    return c.redirect(settingsUrl("error", "Google authorization was not completed.", state?.agentId, employeeFlow));
  }
  try {
    ...
    if (state.employeeId) await connectGoogleCalendarAccount(state.agentId, refreshToken, account, state.employeeId, state.userId);
    else await connectGoogleCalendarAccount(state.agentId, refreshToken, account);
    return c.redirect(settingsUrl("connected", undefined, state.agentId, employeeFlow));
  } catch {
    console.error("[calendar] OAuth callback failed");
    return c.redirect(settingsUrl("error", "Calendar connection failed. Try again and approve all Calendar permissions.", state.agentId, employeeFlow));
  }
});
