...
const m = vi.hoisted(() => ({ state: 'in_flight', role: 'manager', calendarConnected: true, calendarError: false }))
...
vi.mock('@tanstack/react-query', () => ({
  ...
  useQuery: ({ queryKey }: { queryKey: string[] }) => {
    ...
    if (queryKey[1] === 'calendar') return m.calendarError
      ? { isError: true, error: new Error('provider failed') }
      : { data: { connected: m.calendarConnected, events: m.calendarConnected ? [{ id: 'past', calendarId: 'workspace', title: 'Agenda booking', start: today + 'T00:00:00Z', end: today + 'T01:00:00Z' }] : [], sources: [] } }
    return { data: [
...
beforeEach(() => { m.state = 'in_flight'; m.role = 'manager'; m.calendarConnected = true; m.calendarError = false })
