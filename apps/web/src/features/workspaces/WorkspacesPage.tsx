...
export default function WorkspacesPage() {
  ...
  return <main className="min-h-screen bg-stage text-foreground"><header className="border-b border-border bg-card"><div className="mx-auto flex max-w-page items-center justify-between gap-3 px-5 py-4"><Link to="/" className="text-xl font-bold text-primary">DeskRoute</Link><div className="flex items-center gap-3"><span className="hidden text-sm sm:block">{user?.email}</span><Button variant="ghost" onClick={() => void signOut()}>Sign out</Button></div></div></header>
