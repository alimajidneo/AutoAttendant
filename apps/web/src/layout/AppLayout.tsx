...
export default function AppLayout() {
  ...
  const setupLeft = !memberOnly && settings ? setupItems(settings).filter((i) => !i.done).length : 0

  const pendingCount = memberOnly ? 0 : pendingEscalations?.length ?? 0
  const navItems = memberOnly ? MEMBER_NAV_ITEMS : MANAGER_NAV_ITEMS
      ...
      <Sidebar collapsible="icon">
        <SidebarHeader>
          ...
          <div className="flex h-9 items-center gap-2">
            <Link to="/" className="flex min-w-0 items-center gap-2" aria-label="DeskRoute home">
