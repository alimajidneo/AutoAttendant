...
async function inspect(agentId: string, input: Interval, snapshot?: Awaited<ReturnType<typeof getEmployee>>): Promise<{ decision: Decision;
  ...
  if (policy.authority === 'calcom') {
    ...
    const eventType = (await client.eventTypes()).find(t => t.id === policy.eventTypeId);
    if (!eventType || eventType.lengthInMinutes * 60000 !== end.getTime() - start.getTime()) return { decision: unknown };
    const slots = await client.slots(policy.eventTypeId, input.start, input.end, employee.timezone);
  ...
  const results = await Promise.all([...grouped].map(async ([connectionId, ids]) => {
    ...
    const [calendars, ranges] = await Promise.all([
      listProviderCalendars(credential.provider, credential.token),
      fetchProviderBusyRanges(credential.provider, credential.token, [...ids], input.start, input.end),
    ]);
...
export async function bookEmployeeAppointment(agentId: string, input: Interval & { callerName: string; callerPhone?: string; callerEmail?: string; callerConfirmed?: boolean; purpose?: string }, invocationKey?: string) {
  try {
    ...
    const { decision, booking, employee, calcom } = await inspect(agentId, input, snapshot);
    ...
    try {
      if (calcom) {
        const result = await calcom.client.book({ start: input.start, eventTypeId: calcom.eventTypeId,
          ...
          metadata: { agentId, employeeId: employee.id, appointmentId: reservation.id } });
